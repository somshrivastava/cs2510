...
class ConsLoWord implements ILoWord {
   ...
   * ... this.rest.interleave(ILoWord other) ...           -- ILoWord
   * ... this.rest.merge(ILoWord other) ...                -- ILoWord
...
class ExamplesWordLists {
  ...
  //test isEmpty in ActiveWord
  boolean testActiveWordIsEmpty(Tester t) {
    return t.checkExpect(new ActiveWord("", 0, 0).isEmpty(), true)
        && t.checkExpect(new ActiveWord("hello", 0, 0).isEmpty(), false)
        && t.checkExpect(new ActiveWord(" ", 0, 0).isEmpty(), false);
  }

  //test startsWith in ActiveWord
  boolean testActiveWordStartsWith(Tester t) {
    return t.checkExpect(new ActiveWord("hello", 0, 0).startsWith("h"), true)
        && t.checkExpect(new ActiveWord("hello", 0, 0).startsWith("H"), false)
        && t.checkExpect(new ActiveWord("hello", 0, 0).startsWith("w"), false);
  }

  //test reduce in ActiveWord
  boolean testActiveWordReduce(Tester t) {
    return t.checkExpect(new ActiveWord("hello", 0, 0).reduce(), new ActiveWord("ello", 0, 0))
        && t.checkExpect(new ActiveWord("h", 0, 0).reduce(), new ActiveWord("", 0, 0))
        && t.checkExpect(new ActiveWord("", 0, 0).reduce(), new ActiveWord("", 0, 0));
  }

  //test compareTo in ActiveWord
  boolean testActiveWordCompareTo(Tester t) {
    return t.checkExpect(new ActiveWord("apple", 0, 0).compareTo(new ActiveWord("banana", 0, 0)),
        -1)
        && t.checkExpect(new ActiveWord("banana", 0, 0).compareTo(new ActiveWord("apple", 0, 0)), 1)
        && t.checkExpect(new ActiveWord("Apple", 0, 0).compareTo(new ActiveWord("apple", 0, 0)), 0);
  }

  //test draw in ActiveWord
  boolean testActiveWordDraw(Tester t) {
    WorldScene scene = new WorldScene(500, 500);
    WorldScene expected = scene.placeImageXY(new TextImage("hello", 20, Color.BLUE), 100, 100);
    return t.checkExpect(new ActiveWord("hello", 100, 100).draw(scene), expected);
  }

  //test isEmpty in InactiveWord
  boolean testInactiveWordIsEmpty(Tester t) {
    return t.checkExpect(new InactiveWord("", 0, 0).isEmpty(), true)
        && t.checkExpect(new InactiveWord("hello", 0, 0).isEmpty(), false)
        && t.checkExpect(new InactiveWord(" ", 0, 0).isEmpty(), false);
  }

  //test startsWith in InactiveWord
  boolean testInactiveWordStartsWith(Tester t) {
    return t.checkExpect(new InactiveWord("hello", 0, 0).startsWith("h"), false)
        && t.checkExpect(new InactiveWord("hello", 0, 0).startsWith("H"), false)
        && t.checkExpect(new InactiveWord("world", 0, 0).startsWith("w"), false);
  }

  //test reduce in InactiveWord
  boolean testInactiveWordReduce(Tester t) {
    return t.checkExpect(new InactiveWord("hello", 0, 0).reduce(), new InactiveWord("hello", 0, 0))
        && t.checkExpect(new InactiveWord("a", 0, 0).reduce(), new InactiveWord("a", 0, 0))
        && t.checkExpect(new InactiveWord("", 0, 0).reduce(), new InactiveWord("", 0, 0));
  }

  //test compareTo in InactiveWord
  boolean testInactiveWordCompareTo(Tester t) {
    return t.checkExpect(
        new InactiveWord("apple", 0, 0).compareTo(new InactiveWord("banana", 0, 0)), -1)
        && t.checkExpect(
            new InactiveWord("banana", 0, 0).compareTo(new InactiveWord("apple", 0, 0)), 1)
        && t.checkExpect(new InactiveWord("Apple", 0, 0).compareTo(new InactiveWord("apple", 0, 0)),
            0);
  }

  //test draw in InactiveWord
  boolean testInactiveWordDraw(Tester t) {
    WorldScene scene = new WorldScene(500, 500);
    return t.checkExpect(new InactiveWord("hello", 100, 100).draw(scene), scene);
  }
  
  //test sort in MtLoWord
  boolean testMtLoWordSort(Tester t) {
    return t.checkExpect(new MtLoWord().sort(), new MtLoWord());
  }

  //test isSorted in MtLoWord
  boolean testMtLoWordIsSorted(Tester t) {
    return t.checkExpect(new MtLoWord().isSorted(), true);
  }

  //test interleave in MtLoWord
  boolean testMtLoWordInterleave(Tester t) {
    ILoWord list = new ConsLoWord(new ActiveWord("hello", 0, 0),
        new ConsLoWord(new ActiveWord("world", 0, 0), new MtLoWord()));
    return t.checkExpect(new MtLoWord().interleave(list), list)
        && t.checkExpect(list.interleave(new MtLoWord()), list);
  }

  //test merge in MtLoWord
  boolean testMtLoWordMerge(Tester t) {
    ILoWord sortedList = new ConsLoWord(new ActiveWord("hello", 0, 0),
        new ConsLoWord(new ActiveWord("world", 0, 0), new MtLoWord()));
    return t.checkExpect(new MtLoWord().merge(sortedList), sortedList)
        && t.checkExpect(sortedList.merge(new MtLoWord()), sortedList);
  }

  //test checkAndReduce in MtLoWord
  boolean testMtLoWordCheckAndReduce(Tester t) {
    return t.checkExpect(new MtLoWord().checkAndReduce("h"), new MtLoWord());
  }

  //test addToEnd in MtLoWord
  boolean testMtLoWordAddToEnd(Tester t) {
    return t.checkExpect(new MtLoWord().addToEnd(new ActiveWord("hello", 0, 0)),
        new ConsLoWord(new ActiveWord("hello", 0, 0), new MtLoWord()));
  }

  //test filterOutEmpties in MtLoWord
  boolean testMtLoWordFilterOutEmpties(Tester t) {
    return t.checkExpect(new MtLoWord().filterOutEmpties(), new MtLoWord());
  }

  //test draw in MtLoWord
  boolean testMtLoWordDraw(Tester t) {
    WorldScene scene = new WorldScene(500, 500);
    return t.checkExpect(new MtLoWord().draw(scene), scene);
  }
  ...
  boolean testSort(Tester t) {
    ...
    return t.checkExpect(mt.sort(), mt) 
        ...
        // sorting a list with uppercase and lowercase strings
  ...
  boolean testFilterOutEmpties(Tester t) {
    ...
    return t.checkExpect(mt.filterOutEmpties(), mt)
        ...
        && t.checkExpect(
            new ConsLoWord(empty, new ConsLoWord(empty, activeList)).filterOutEmpties(),
            activeList);
  }
