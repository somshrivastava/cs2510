  ...
  void testInitBoard(Tester t) {
    ...
    for (int col = 0; col < 4; col++) {
      for (int row = 0; row < 4; row++) {
        ...
        if (p1.top != p2.top || p1.bottom != p2.bottom || p1.left != p2.left
            || p1.right != p2.right) {
  ...
  void testMakeScene(Tester t) {
    ...
    for (ArrayList<GamePiece> column : game.board) {
      for (GamePiece piece : column) {
        expectedScene.placeImageXY(piece.tileImage(), piece.column * TILE_SIZE + TILE_SIZE / 2,
    ...
    // draw status bar
    expectedScene.placeImageXY(new RectangleImage(sceneWidth, 40, OutlineMode.SOLID, Color.BLACK),
        sceneWidth / 2, sceneHeight - 20);

    expectedScene.placeImageXY(new TextImage("Steps: 3 | Time: 2s", 15, Color.WHITE),
        sceneWidth / 2, sceneHeight - 20);
